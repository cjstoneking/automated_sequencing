...
#rotates and shifts a 2D image
#keeps the dimensions the same
#takes all clipping into account
def rotate_and_shift(img, angle=0, shift=[0,0]):
    ...
    orig_i1 = orig_i1 + shift[0]
    orig_j0 = orig_j0 + shift[1]
    orig_j1 = orig_j1 + shift[1]
    ...
    if(orig_i1 > rotated_img.shape[0]):
        d = rotated_img.shape[0] - orig_i1
        orig_i1 = rotated_img.shape[0]
        trans_i1  = trans_i1 + d
    if(orig_j1 > rotated_img.shape[1]):
        d = rotated_img.shape[1] - orig_j1
        orig_j1 = rotated_img.shape[1]
        trans_j1  = trans_j1 + d
